...
int main(int argc, char* argv[]) {
	...
	for (int workload_index = 0; workload_index < num_workloads; workload_index++) {
		...
		sprintf(init_cond_arrival, "../initial_conditions/inter_arrival_%d.bin", workload_index);
		sprintf(init_cond_id, "../initial_conditions/kernel_id_%d.bin", workload_index);
		sprintf(init_cond_executions, "../initial_conditions/num_executions_%d.bin", workload_index);

		// Reading initial conditions binary files generated by python script
		// read_binary_file("../initial_conditions/inter_arrival.bin", (void**) &inter_arrival_buffer);
		// read_binary_file("../initial_conditions/kernel_id.bin", (void**) &kernel_label_buffer);
		// read_binary_file("../initial_conditions/num_executions.bin", (void**) &num_executions_buffer);
